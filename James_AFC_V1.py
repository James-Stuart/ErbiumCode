...
def decay_per_SpecAnSweep(burn_f,n=50):
    ''' Burn a hole and look at the anti-hole as the SpecAn sweeps across it '''
    burn_sequence_AWG(burn = 200*ms, burn_freq= burn_f, record = 'False', rec_freq = freq, rec_span = 20*MHz, f_name = ' ')
    
    SpecAn.write("CF " + str(burn_f-(107.64)*MHz))
    SpecAn.write("SP " + str(10*MHz))
    dat = stuff.run(n)
    file1 = open("C:\Users\Milos\Desktop\James\\SpecAnSweepDecay.txt",'a')
    np.savetxt(file1, dat, fmt='%.10e')
    file1.close()
#
...
#
def burn_sequence_AWG(burn, burn_freq, record = 'True', rec_freq = '', rec_span = 10*MHz, f_name = '', nu = 2, sa = 'Y'):
    ...
    if record == 'True':
        ...
        hb.run_offset(SpecAn, freq = rec_freq, span = rec_span, res = 30*kHz, sweep = 50*ms, full_span = 'N', show_window = 'N',n = nu)
...
def burn_sequence_AWG_cust(burn, burn_freq, record = 'True', rec_freq = '', rec_span = 10*MHz, f_name = '', nu = 2, sa = 'Y', rec_offs = True, opt_bac = 'offs1.txt'):
...
##     record_dm_1(burn_f, hyperfine + ' ' + file_suff, tn = teeth)
    ...
    t_s = 4*ns
    r_l = 50*us
    t_sample,res = pico.run_rapid_block(ps, Vrange = 0.5, n_captures = shots, t_sample = t_s, record_length = r_l)
    n_data = r_l/t_s
    sleep(0.1)
...
def hardburn():
    ...
    spin_pump_seq()
    mc.setWvfm(2)
    burn_sequence_AWG_cust(burn = 50*ms, burn_freq= freq, record = 'True', rec_span = span, f_name = ' ' + title_str, sa = 'N')# ' + str(i))
    
    filepath = hb.create_file(SpecAn, compensated = 'Y', n=1, burn_time = 0, filename = ' Spin pumped after ' + title_str)
    sleep(100*ms)
    pb.Sequence([   (['ch5'], 0.5*s),
                    (['ch2','ch4','ch5'], 50*ms), #RF Switch, Trigger SpecAn, EOM Bias on 
                    (['ch2','ch5'], 100*ms),
                    ], loop=False)
    hb.record_trace(SpecAn, filepath, filename = ' Spin pumped after ' + title_str, compensated = 'Y', sweep_again ='N', n=1, burn_time = '') 
...
##     ITLA.SetFrequency(194.9437) #THz magnetic sub group 
    ...
    sleep(5)
    filepath = hb.create_file(SpecAn, n=1, burn_time = 0, filename = f_name) #Record spin pumping after site 1 burn
    pb.hole_burn(1)
    filepath=hb.record_trace(SpecAn, filepath, compensated = 'Y', sweep_again = 'Y', burn_time = 1)
    
    fg.setOutputState(bOn=1) #Turns Channel 2 back on so we can spin pump again
...
def PhotonicsLaser():    
    ...
    #Set Laser to usual parameters for Spin Pumping
##     ITLA.SetFrequency(194.9412) #THz, Site 2
##     ITLA.SetFrequency(194.9412) #THz, Site 2
##     ITLA.SetFrequency(195.162) #THz, Site 1
##     ITLA.SetPower(1600) #dBm * 100    
    
##     ITLA.SetSweepRange(1) #20GHz
##     ITLA.SetSweepRate(20000)#20,000MHz    
...
def spinpump_delay(f_name = 'delayed recording of spinpump', delay = 20, secondLpath = False, latchT = 5):
    spin_pump_seq(spintime = 10*s,SpecAnSweep = 'Y',rec ='Y') #Spin pump site 2
    if secondLpath:
...
def temp_func():
    ...
    for i in range(10):
        filepath = hb.create_file(SpecAn, compensated = 'Y', n=1, burn_time = 0, filename = ' 2nd background ' + str(i))
        sleep(100*ms)
        pb.Sequence([   (['ch5'], 0.5*s),
                        (['ch5', 'ch6'], 20*us), (['ch5'], 5*ms),
                        (['ch2','ch4','ch5'], 50*ms), #RF Switch, Trigger SpecAn, EOM Bias on 
        ...
        hb.record_trace(SpecAn, filepath, filename = ' 2nd background ' + str(i), compensated = 'Y', sweep_again = 'Y', n=1, burn_time = '')
