################################################################################
###									     ###
###		JAMES HOLE BURNING TESTING STUFF V3.0                        ###
###									     ###
################################################################################
...
def record_trace(SpecAn, filepath, filename = '', compensated = 'Y', sweep_again = 'Y', n=1, burn_time = '', opt_bac = ''):
...
def compensate(data, span, opt_back =''):
...
def run_offset(SpecAn, freq = 1.45*GHz, span = 2.9*GHz, res = 30*kHz, sweep = 50*ms, full_span = 'Y', show_window = 'N',n = 20):
...
def windfreak(freq, dB, power_state, filepath, talk = 'YES', write = 0):
    ...
    wf.Windfreak_ONOFF(1, talk, instr)
...
def burn_sequence(burn, burn_freq, power, hl = 'low', record = 'True', rec_freq = '', rec_span = 10*MHz, f_name = ''):
    ...
    if record == 'True':
        run_offset(SpecAn, freq = rec_freq, span = rec_span, res = 30*kHz, sweep = 50*ms, full_span = 'N', show_window = 'N')
        filepath = create_file(SpecAn, compensated = 'Y', n=1, burn_time = burn, filename = f_name)
    ...
    if record == 'True':
        filepath=record_trace(SpecAn, filepath, compensated = 'Y', burn_time = burn, filename = f_name)
    sleep(0.5*s)
...
##     full_sweep(SpecAn)
    HP8560E_SpecAn_Trigger('FREE', 'CONTS', SpecAn)
##     SIH.free_run_plot_window_old('Y',full_span = 'Y')
##     quick_rec(SpecAn, '18-07-19 Power loss through SMA setup')
    
    
##     filepath = create_file(SpecAn, compensated = 'N', n=1, burn_time = 0, filename = 'det2fresponse')
##     pb.Sequence([(['ch5'], 5*s)] + [(['ch2','ch5','ch4'], 2.1*s)] + [(['ch2','ch5'], 100*ms)], loop=False)
##     record_trace(SpecAn, filepath, compensated = 'N', n=1, burn_time = 0)
##     print SpecAn.read()


##     SpecAn.write("CLEAR; RQS 4")
##     pb.Sequence([(['ch4'], 125*ms),([], 125*ms)], loop=True)
##     HP8560E_SpecAn_Trigger('FREE', 'CONTS', SpecAn)
##     print(SpecAn.query('CF?'))
    
##     full_sweep(SpecAn)        
##     run_offset(SpecAn, show_window = 'N')#Dont run this mid experiment, idiot...
##     SIH.free_run_plot_window('Y',full_span = 'Y')
