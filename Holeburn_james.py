...
def record_trace(SpecAn, filename = '', compensated = 'N', sweep_again = 'Y', n=1, burn_time = ''):
...
##     
    ...
    #Write other header information here!
    optional_header = ''
    ...
    np.savetxt(filepath, data, fmt='%.10e')
    sleep(0.01)
...
def spin_polarise(SpecAn, SpecAn_Bool, start_freq, direction = 'backwards', pumps = 10):
    ...
    delta m = +/-1. 
    MIN FREQ: 875MHz'''
    sweep_array = np.array([[start_freq, start_freq + 600*MHz, 500*ms]])
    print np.min(sweep_array)
    print np.max(sweep_array)
    [Stanford_FG, Stanford_Bool] = stan.Initialise_Stanford_FG()
    ...
    if direction == 'backwards':
        [num_array, waveform_length] = stan.VCO_Sweep_backwards(sweep_array, Stanford_FG, Stanford_Bool, 'op amp', SpecAn, SpecAn_Bool)
...
##     run_offset(SpecAn, freq = 2.560*GHz, span = 1*MHz, res = 30*kHz, sweep = 50*ms, full_span = 'N', show_window = 'N')
##     pb.hole_burn(burn)
##     record_trace(SpecAn, compensated = 'Y', n=1, burn_time = burn)
##     sleep(3*s)

##     #Rempump will only go as low as 875 MHz
##     repump_sequence(SpecAn, SpecAn_Bool, 1.2*GHz, 2.2*GHz)
##     sleep(0.5)
##     #Full 2.9GHz scan
##     full_sweep(SpecAn)
##     SIH.free_run_plot_window('Y',full_span = 'Y')    
...
#================ Hole Burn then sit on edge of hole =====================
    burn = 0.02*s
    freq1 = 2.560*GHz
    freq2 = freq1 + 50*kHz
    #Hole burning sequence
    run_offset(SpecAn, freq = freq1, span = 2*MHz, res = 30*kHz, sweep = 50*ms, full_span = 'N', show_window = 'N')
    pb.hole_burn(burn)
    record_trace(SpecAn, compensated = 'Y', n=1, burn_time = burn)
    sleep(0.5*s)
...
#================== SPIN PUMPING ===============================================
##     for i in range(3):
##         spin_polarise(SpecAn, SpecAn_Bool, 1250*MHz, direction = 'forwards', pumps = 20)
##         sleep(10)
##     SIH.free_run_plot_window('Y',full_span = 'Y')
