...
##         burn_freq = 2.525*GHz
    ...
    run_offset(SpecAn, freq = burn_freq-100*MHz, span = 300*MHz, res = 30*kHz, sweep = 50*ms, full_span = 'N', show_window = 'N')
...
def ladder_burn():
    ''' Spin polarize crystal into |7/2>. Burn on the m = -1 to make a |5/2>
    anti hole. Burn on that hole to shift to |3/2>.
    '''
    spin_pump_seq(2.4*GHz,800,record = "False")
    burn_sequence(0.05,1.2*GHz,-15,hl = 'high')
    sleep(1)
    SIH.free_run_plot_window('N',full_span = 'N')
