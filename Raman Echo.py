...
for j in range (1, 2):
    for i in range(1, 30):
        ...
        pb.Sequence([(['ch1'], 11*s),                                   #Spin Pump with VCO
                    ...
                    ([],pulse_delay*ms),                                       #wait fore dephasing
                    ([], 15*us),
                    (['ch1', 'ch2'], 4*us),  
                    (['ch1', 'ch2', 'ch4'], 2*us),                      #set of three Pi pulses to rephase
                    (['ch1', 'ch2'], 4*us),  
                    ([],pulse_delay*ms),                                       #wait for rephasing
                    (['ch7', 'ch1', 'ch2', 'ch3','ch6'], 60*us),        #switch light to carrier, turn on weak probe and trigger scope 
                    ([], 1*ms)],
                    loop=False)
        ...
        folder_name = 'Raman Echoes, 994.7 Mhz'
